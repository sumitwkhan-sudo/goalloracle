...
**Email constraints (every engagement email):** embed the GoalOracle logo (small header, not a banner); sign off exactly `- Sumit, Founder of GoalOracle.io` (operator dropped "and Football Lover", Jul 2026); authenticate the domain (SPF/DKIM/DMARC) + keep emails personal/low-image for Primary placement.
